...
def _read_and_replace(_trace_file):
    ...
    if os.path.exists(_trace_file):
        ...
        for s in lines:
            ...
            #deal with function
            it = re.finditer(r'([a-zA-Z]+\.)*publishModel [a-zA-Z]+', s)
            for match in it:
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--file", help="replace file")
    args = parser.parse_args()

    _trace_file = args.file

    if os.path.isdir(_trace_file):
        dealing_with_dir(_trace_file)
    else:
